...
Key features include real-time waste tracking with GPS-based location services, photo documentation for waste verification, integrated payment processing via Razorpay, a carbon credit tracking system that quantifies environmental impact, and a gamification-based rewards system to incentivize participation. The platform also integrates AI capabilities through Google Genkit for intelligent waste classification.
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const options = {
        ...
        name: 'AgriTrace',
        description: 'Waste Purchase Payment',
...
\section{Razorpay Payment API Route}

\noindent The Next.js API route handles server-side order creation and verification:

\begin{lstlisting}[caption={Payment Order API (api/payment/create-order/route.ts)}, label={lst:paymentapi}]
...
export async function POST(request: Request) {
  try {
    const { amount, listingId, buyerId, sellerId } =
      await request.json();
...
\noindent The API route uses server-side Razorpay credentials (never exposed to the client). The order amount is converted from rupees to paise (multiply by 100) as required by the Razorpay API. A corresponding Firestore order document is created to maintain an internal record of all transactions.
...
\begin{itemize}
    ...
    \item \textbf{IN\_TRANSIT:} ``Mark Delivered'' button transitions the listing to DELIVERED, with an optional delivery proof photo upload
...
\begin{enumerate}
    ...
    \item[{[3]}] L. Moroney, ``\textit{The Definitive Guide to Firebase},'' Apress, 1st Edition, 2017.
    ...
    \item[{[11]}] Ministry of Agriculture and Farmers Welfare, ``\textit{National Policy for Management of Crop Residues},'' Government of India, 2014.

    \item[{[12]}] Indian Agricultural Research Institute (IARI), ``\textit{Crop Residue Management: Challenges and Solutions},'' IARI Publication, New Delhi, 2019.
    ...
    \item[{[21]}] B. W. Barry, ``\textit{Constructive Cost Model -- COCOMO},'' \textit{Software Engineering Economics}, Prentice Hall, 1981.
    ...
    \item[{[25]}] Central Pollution Control Board (CPCB), ``\textit{Air Quality Monitoring Data -- National Air Quality Index},'' Government of India, 2023.
\end{enumerate}
